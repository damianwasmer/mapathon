...
const ManagePage = (props) => {
    ...
    if(groupnr == 10 && filterusr){
        ...
        tagsnew = tags.filter(tag => tag.Creator.name == [usr.user.name] && tag.group == [groupnr]);
        categoriesnew = categories.filter(categorie => categorie.Creator.name == [usr.user.name] && categorie.group == [groupnr]);
    } else if (groupnr != 10 ){
